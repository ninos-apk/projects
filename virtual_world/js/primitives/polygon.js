class Polygon{
    ...
    static break(poly1, poly2) {
        ...
        for (let i = 0; i < segs1.length; i++){
            for (let j = 0; j < segs2.length; j++){
                const intersec = getIntersection(
                    segs1[i].p1, segs1[i].p2, segs2[j].p1, segs2[j].p2
                )
                if (intersec && intersec.offset != 1 && intersec.offset != 0) {
                    const point = new Point(intersec.x, intersec.y);
                    let aux = segs1[i].p2;
                    segs1[i].p2 = point;
    ...
    containsPoint(point) {
        ...
        for (const seg of this.segments) {
            const intersec = getIntersection(outerPoint, point, seg.p1, seg.p2);
            if (intersec) {
                intersectionCount++;
            }
        }
